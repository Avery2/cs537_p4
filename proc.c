...
void appendRR(struct proc *p){
	tail = head == 0  ?(head =p) :(tail->next = p);
}
...
yield(void)
{
  ...
  myproc()->runtime++;
  myproc()->schedticks++;
  if(myproc()->runtime >= myproc()->slice +myproc()->comp ){
	  myproc()->comp =0;
  	myproc()->state = RUNNABLE;
  	appendRR(myproc());
	sched();
  }else if(myproc() ->runtime >= myproc()->slice){
		myproc()->compticks ++;
  }
...
struct proc *p;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == SLEEPING && p->chan == chan){
	    if(p->chan == &ticks &&  p->sleepat + p->sleepfor > ticks){
		    continue;
	    }
		p->comp = ticks- p->sleepat;
		p->sleepticks += p->comp;
      p->state = RUNNABLE;
      appendRR(p);
    }
